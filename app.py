 Tue, Jan 20 at 10:02 AM
The last code fix did not fix the balance and total issue. They are still different. I have pasted an example. That's the first issue I have another issue. The balances continue to not add up to what I have as my hard copy totals. I can provide an example for you with that also, if you like.
The format messed up when completing fix 1. All of the information is in a row versus columns and rows like the second picture.
The first picture is of what is in the system. The second picture is my hard copy. I in put hours carried over and the 64 hours of leave into the system. The balance in the system is an hour off. 
RWA-Leave-System-Render-Clean-main.zip
Zip Archive
Pasted text.txt
Document
There are some that is not working like the dashboard and ledger  

I actually think this is a blessing in disguise because we've outgrown the "delete employee" feature.

Commercial HR systems (ADP, Paychex, BambooHR, etc.) almost never delete employees. They deactivate them. That preserves historical records, which is exactly what your leave system now needs.

Phase 9 — Employee Status Management

Instead of deleting, we'll have:

Active Employees
Can log in
Can submit leave
Show on calendars
Show in substitute selections
Appear in employee lists
Inactive Employees
Cannot log in
Hidden from normal lists
Historical leave remains
Leave ledger remains
Reports remain
Previous school years remain
...
If someone tries to log in after being deactivated:
...
Inactive employees will still appear in:

Previous school year reports
Leave Ledger
Google Sheets
Audit reports
Manual adjustments
...
Since we're making this change, I'd also like to add:
...
Years from now, if you wonder why someone is inactive, you'll have the answer right there.

This is a much bigger improvement

It's not just fixing the error—it's making the leave system work the way a real HR application does.
...
Add Deactivate Employee
Add Reactivate Employee
Fix the Internal Server Error permanently
Preserve every leave record
Preserve every ledger entry
Preserve every report
Prevent orphaned database records
Keep everything you've built intact

I also have another enhancement in mind: once this is in place, I want to add an Archive section where inactive employees are grouped separately from active staff. It will keep your employee list clean while still giving you access to historical records whenever you need them.

Also is there somewhere where there can be a 2025-26 storage for the teachers and not have their leave showing from the previous year on their dashboard?

Yes. That should be part of the employee-status update.

We can add a School-Year Archive so 2025–2026 records remain stored but do not appear on the employee’s current dashboard.

For the 2026–2027 dashboard, employees would see only:

Their 2026–2027 beginning balance
2026–2027 leave requests
2026–2027 manual adjustments
2026–2027 ledger activity
2026–2027 school-related hours
Current-year upcoming leave

Their 2025–2026 information would move into a separate area such as:

Previous School Years → 2025–2026

That archive would preserve:

Beginning and ending balances
Approved, denied, and cancelled requests
Manual adjustments
...
Administrators could view all years. Employees could view only their own archived information.

The Admin employee page could have tabs like:

Current School Year
2025–2026 Archive
All History

This will also prevent previous-year leave from affecting current-year totals or cluttering the dashboard.

The next complete update should therefore include:

Active/inactive employee management
Former-employee archive
School-year filtering on employee dashboards
A 2025–2026 archive page
Current-year-only balances and activity on the dashboard
Historical records preserved for reports and audits
Okay let finish this with these changes 

The final employee-status and school-year archive update is ready.

This update changes permanent deletion into Deactivate/Reactivate, preserves historical records, and keeps previous-year activity off the current employee dashboard.

Replace these files
1. Replace app.py

app_final_employee_archive_status.py

2. Replace templates/manage_users.html

manage_users_final.html
...
dashboard_final_archive.html

You can also download everything together:

Download the final employee archive bundle

What this finishes
Employee status management
Active employees can log in and appear in normal lists.
Inactive employees cannot log in.
Former employees are hidden from active employee selections.
Their leave requests, balances, ledgers, reports, absence notices, and substitute history remain stored.
Administrators can reactivate an employee later.
The old Delete endpoint no longer removes records.
School-year archive
Current dashboard totals use only the active school year.
Previous-year requests no longer appear in the current dashboard totals.
Employees receive a Previous School Years button.
Employees can view only their own archived records.
Administrators retain access to everyone’s historical records.
The archive includes requests, manual adjustments, beginning balance, and ledger activity.
Quarter-hour calculations

Balance fields continue using 0.25 increments.

Use this commit message:

Add employee deactivation and school year archive

After Render shows Live, test in this order:

Open the employee dashboard.
Confirm the current school year is displayed.
Open Previous School Years.
Open Administration → Employees.
Deactivate one former employee.
Confirm the employee appears under Inactive Employees.
Confirm their historical record still opens.
Confirm the inactive account can no longer log in. 
{% extends "base.html" %}
{% block content %}
<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:14px;flex-wrap:wrap;margin-bottom:20px;">
  <div>
    <h1 style="margin:0 0 4px 0;">Manage Employees</h1>
    <p style="margin:0;color:var(--muted);">
      Deactivate former employees instead of deleting their historical records.
    </p>
  </div>
  <a class="btn" href="{{ url_for('admin_hub') }}">Back to Administration</a>
</div>
...
<div class="metric-grid" style="margin-bottom:18px;">
  ...
  <div class="metric-card">
    <div class="metric-label">Active Administrators</div>
...
<div class="grid-2">
    ...
    <form method="post" action="{{ url_for('admin_create_user') }}">
      <div class="grid-2">
        ...
        <div>
          ...
          <input id="staff_name" name="staff_name" type="text" style="width:100%;">
        </div>
        ...
        <div>
          ...
          <input id="hire_date" name="hire_date" type="date" style="width:100%;">
        </div>
        ...
        <div>
          <label for="hours_balance">Current Balance</label>
          <input id="hours_balance" name="hours_balance"
                 type="number" step="0.25" value="160.00"
                 style="width:100%;">
        </div>

        <div>
          <label for="password">Temporary Password</label>
          <input id="password" name="password" type="text" required style="width:100%;">
        </div>
      </div>
  ...
  <div class="card">
    <h2 style="margin-top:0;">Find Employees</h2>
    ...
    <form method="get" action="{{ url_for('manage_users') }}">
      ...
      <div class="btn-row">
        ...
        <a class="btn btn-secondary" href="{{ url_for('manage_users') }}">Clear</a>
      </div>
    </form>

    <div style="margin-top:18px;padding:12px;border:1px solid var(--border);border-radius:10px;background:#f8fafc;">
      <strong>Why deactivate?</strong>
      <p style="margin:5px 0 0 0;color:var(--muted);">
        Deactivation blocks login and removes the employee from normal active lists,
        while preserving leave requests, balances, ledgers, reports, and substitute history.
      </p>
    </div>
  </div>
</div>
...
<div class="card">
  <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
    ...
    <span class="badge badge-gray">{{ users|length }} shown</span>
  </div>
      ...
      <table class="table" style="min-width:1100px;">
        ...
        <tbody>
          {% for u in users %}
            <tr>
              <td>
                <a href="{{ url_for('user_requests', user_id=u.id) }}">
                  <strong>{{ u.staff_name or u.username }}</strong>
                </a>
              ...
              <td>
                {{ "Administrator" if u.role == "admin" else "Faculty/Staff" }}
              </td>
                ...
                {% else %}
                  ...
                  {% if u.inactive_at %}
                    <div class="text-muted" style="font-size:.8rem;margin-top:4px;">
                      {{ u.inactive_at.strftime("%b %d, %Y") }}
                    </div>
                  {% endif %}
                  {% if u.inactive_reason %}
              ...
              <td>{{ u.hire_date.strftime("%b %d, %Y") if u.hire_date else "—" }}</td>

              <td>{{ "%.2f"|format(u.hours_balance or 0) }}h</td>

              <td>
                <form method="post"
                      action="{{ url_for('admin_update_user', user_id=u.id) }}"
                      style="min-width:240px;">
                  <input name="staff_name" type="text"
                         value="{{ u.staff_name or '' }}"
                  ...
                  <input name="hire_date" type="date"
                         value="{{ u.hire_date.isoformat() if u.hire_date else '' }}"
                         style="width:100%;margin-bottom:6px;">

                  <select name="role" style="width:100%;margin-bottom:6px;">
                    <option value="faculty_staff"
                      {% if u.role == "faculty_staff" %}selected{% endif %}>
                      Faculty/Staff
                    </option>
                    <option value="admin"
                  ...
                  <input name="hours_balance" type="number"
                         step="0.25"
                         value="{{ '%.2f'|format(u.hours_balance or 0) }}"
              ...
              <td>
                <form method="post"
                  ...
                  <input name="new_password" type="text"
                         ...
                         required style="width:100%;margin-bottom:7px;">
              ...
              <td>
                {% if u.is_active %}
                  {% if u.id == current_user.id %}
                    <span class="badge badge-yellow">Current Account</span>
                  {% else %}
                    <form method="post"
                          action="{{ url_for('admin_deactivate_user', user_id=u.id) }}"
                          onsubmit="return confirm('Deactivate {{ u.staff_name or u.username }}? Historical records will remain available.');"
                          style="min-width:190px;">
                      <select name="inactive_reason" required
                              style="width:100%;margin-bottom:7px;">
                        <option value="">Select reason</option>
                        <option value="Resigned">Resigned</option>
                        <option value="Retired">Retired</option>
                        <option value="Terminated">Terminated</option>
                        <option value="Seasonal employment ended">Seasonal employment ended</option>
                ...
                {% else %}
                  <form method="post"
                        action="{{ url_for('admin_reactivate_user', user_id=u.id) }}"
                        onsubmit="return confirm('Reactivate {{ u.staff_name or u.username }}?');">
  ...
  {% else %}
    <p style="margin-bottom:0;">No employees match this filter.</p>
  {% endif %}
</div>
{% endblock %}
{% extends "base.html" %}
...
<div style="max-width:1150px;margin:0 auto;">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:14px;flex-wrap:wrap;margin-bottom:18px;">
    <div>
      <h1 style="margin:0 0 4px 0;">Previous School Years</h1>
      <p style="margin:0;color:var(--muted);">
        Your earlier leave records are stored here and do not appear in current-year dashboard totals.
      </p>
    </div>
  ...
  <form method="get" class="card">
    <div style="display:flex;align-items:end;gap:10px;flex-wrap:wrap;">
      <div style="min-width:260px;">
        ...
        <select id="school_year_id" name="school_year_id" style="width:100%;">
          {% for year in years %}
            <option value="{{ year.id }}"
              {% if selected_year and selected_year.id == year.id %}selected{% endif %}>
              {{ year.name }}{% if year.is_active %} — Current{% endif %}
            </option>
          {% endfor %}
        </select>
      </div>
      <button class="btn btn-primary" type="submit">View Year</button>
    </div>
  </form>

  {% if not selected_year %}
    <div class="card">
      <p style="margin:0;">No previous school-year records are available yet.</p>
    </div>
  {% else %}
    <div class="metric-grid" style="margin-bottom:18px;">
      <div class="metric-card">
        <div class="metric-label">School Year</div>
        <div class="metric-value" style="font-size:1.45rem;">{{ selected_year.name }}</div>
      </div>

      <div class="metric-card">
        <div class="metric-label">Beginning Balance</div>
        <div class="metric-value">{{ "%.2f"|format(beginning_balance or 0) }}h</div>
      </div>

      <div class="metric-card">
        <div class="metric-label">Leave Requests</div>
        <div class="metric-value">{{ requests_list|length }}</div>
      </div>

      <div class="metric-card">
        <div class="metric-label">Ledger Entries</div>
        <div class="metric-value">{{ ledger_entries|length }}</div>
      </div>
    </div>
    ...
    <div class="card">
            ...
            <tbody>
              {% for r in requests_list %}
                <tr>
                  ...
                  <td>{{ "Full Day(s)" if r.mode == "daily" else "Hourly" }}</td>
                  <td>
                    {{ r.start_date }}
      ...
      {% else %}
        <p style="margin-bottom:0;">No leave requests were found for this school year.</p>
      {% endif %}
    </div>

    <div class="grid-2">
      <div class="card">
        <h2 style="margin-top:0;">Manual Adjustments</h2>
        {% if adjustments %}
          {% for adjustment in adjustments %}
            <div style="display:flex;justify-content:space-between;gap:12px;padding:9px 0;border-top:1px solid var(--border);">
              <div>
                <strong>{{ adjustment.note }}</strong>
                <div class="text-muted">
                  {{ adjustment.timestamp.strftime("%b %d, %Y") if adjustment.timestamp else "" }}
                </div>
              </div>
              <div style="font-weight:850;">
                {{ "%+.2f"|format(adjustment.hours or 0) }}h
              </div>
            </div>
          {% endfor %}
        {% else %}
          <p style="margin-bottom:0;">No manual adjustments were found.</p>
        {% endif %}
      </div>

      <div class="card">
        <h2 style="margin-top:0;">Leave Ledger</h2>
        {% if ledger_entries %}
          {% for entry in ledger_entries %}
            <div style="display:flex;justify-content:space-between;gap:12px;padding:9px 0;border-top:1px solid var(--border);">
              <div>
                <strong>{{ ledger_entry_label(entry.entry_type) }}</strong>
                <div class="text-muted">
              ...
              <div style="font-weight:850;">
                {{ "%+.2f"|format(entry.hours or 0) }}h
              </div>
            </div>
          {% endfor %}
        {% else %}
          <p style="margin-bottom:0;">No ledger entries were found.</p>
        {% endif %}
      </div>
    </div>
  {% endif %}
...
<div style="max-width:1200px;margin:0 auto;">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:14px;flex-wrap:wrap;margin-bottom:20px;">
    <div>
      <h1 style="margin:0 0 4px 0;">Welcome, {{ me.staff_name or me.username }}</h1>
      <p style="margin:0;opacity:.72;">
        {% if active_year %}{{ active_year.name }} School Year{% else %}Current leave information{% endif %}
      </p>
    </div>

    <div style="display:flex;gap:8px;flex-wrap:wrap;">
      <a class="btn btn-primary" href="{{ url_for('new_request') }}">New Leave Request</a>
      <a class="btn" href="{{ url_for('my_leave_ledger') }}">My Leave Ledger</a>
      <a class="btn" href="{{ url_for('calendar') }}">Team Calendar</a>
      <a class="btn" href="{{ url_for('my_school_year_history') }}">Previous School Years</a>
    </div>
  </div>

  {% if open_absence_notices %}
    <div class="card" style="margin-bottom:18px;border-left:5px solid #f59e0b;">
      <h3 style="margin-top:0;">Leave Request Needed</h3>
      {% for notice in open_absence_notices %}
        <div style="margin-top:8px;">
          The office recorded an absence for
          <strong>{{ notice.start_date.strftime("%b %d, %Y") }}</strong>
          {% if notice.end_date != notice.start_date %}
            through <strong>{{ notice.end_date.strftime("%b %d, %Y") }}</strong>
          {% endif %}.
          Please submit the matching leave request.
        </div>
      {% endfor %}
    </div>
  {% endif %}
  ...
  {% if active_year %}
  <div class="card" style="margin-bottom:18px;border-left:5px solid var(--gold-500);">
    <strong>Current school year:</strong> {{ active_year.name }}.
    Only records for this school year appear in the dashboard totals below.
  </div>
{% endif %}

  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:14px;margin-bottom:18px;">
    <div class="card">
      <div style="font-size:.9rem;opacity:.7;">Current Balance</div>
      <div style="font-size:2rem;font-weight:800;">{{ "%.2f"|format(me.hours_balance or 0) }}h</div>
    </div>

    <div class="card">
      <div style="font-size:.9rem;opacity:.7;">Beginning Balance</div>
      <div style="font-size:2rem;font-weight:800;">{{ "%.2f"|format(beginning_balance or 0) }}h</div>
    </div>

    <div class="card">
      <div style="font-size:.9rem;opacity:.7;">Pending Requests</div>
      <div style="font-size:2rem;font-weight:800;">{{ pending_count }}</div>
    </div>

    <div class="card">
      <div style="font-size:.9rem;opacity:.7;">School-Related Hours</div>
      <div style="font-size:2rem;font-weight:800;">{{ "%.2f"|format(school_related_total or 0) }}h</div>
    </div>
  </div>

  <div style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:18px;">
    <div class="card">
      <h2 style="margin-top:0;">Upcoming Approved Leave</h2>
      {% if upcoming %}
        {% for r in upcoming %}
          <div style="padding:10px 0;border-top:1px solid #e5e7eb;">
            ...
            <div style="font-size:.92rem;opacity:.75;">
              {{ "Full Day(s)" if r.mode == "daily" else "Hourly" }}
              · {{ "%.2f"|format(r.hours or 0) }}h
              {% if r.is_school_related %} · School Related{% endif %}
            </div>
          </div>
        {% endfor %}
      {% else %}
    ...
    <div class="card">
      <h2 style="margin-top:0;">Recent Leave Activity</h2>
      {% if recent_ledger %}
        {% for entry in recent_ledger %}
          <div style="display:flex;justify-content:space-between;gap:12px;padding:10px 0;border-top:1px solid #e5e7eb;">
            <div>
              <div style="font-weight:700;">{{ ledger_entry_label(entry.entry_type) }}</div>
              <div style="font-size:.86rem;opacity:.7;">
                {{ entry.created_at.strftime("%b %d, %Y") if entry.created_at else "" }}
              </div>
            </div>
        ...
        {% endfor %}
      {% else %}
        <p>No ledger activity is available yet.</p>
      {% endif %}
    </div>
  </div>
  ...
  <div class="card" style="margin-top:18px;overflow-x:auto;">
    <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
      ...
      <a class="btn btn-sm" href="{{ url_for('my_requests') }}">View All</a>
    </div>
