...
# Make current_year & NAV available to every template
@app.context_processor
def inject_globals():
    NAV = type("Nav", (), {})()
    NAV.dashboard = url_for("dashboard") if current_user.is_authenticated else url_for("login")
    NAV.my_requests = url_for("my_requests") if current_user.is_authenticated else url_for("login")
    NAV.team_calendar = url_for("calendar") if current_user.is_authenticated else url_for("login")
    NAV.new_request = url_for("new_request") if current_user.is_authenticated else url_for("login")
    NAV.admin = url_for("manage_users") if current_user.is_authenticated else url_for("login")
    NAV.logout = url_for("logout") if current_user.is_authenticated else url_for("login")
...
class LeaveRequest(db.Model):
    ...
    # legacy single substitute text (kept for backward-compatibility; optional)
    substitute = db.Column(db.String(120))

    user = db.relationship("User", backref="leave_requests", lazy="joined")

    # NEW: multiple substitutes
...
def ensure_db():
    ...
    try:
        if db.engine.dialect.name == "sqlite":
            # ensure earlier added columns exist
...
def dashboard():
    ...
    return render_template("dashboard.html", title="Dashboard", me=current_user, workday=WORKDAY_HOURS, recent=recent)
...
# ---- School-related toggles (owner or admin; only when pending)
...
# ---- Substitutes (multiple, with hours) — ADMIN only
...
def approve(req_id):
    ...
    flash("Approved.", "success"); return redirect(url_for("my_requests"))
...
def disapprove(req_id):
    ...
    send_email([u.email] + admin_emails(), subj, body)
    flash("Disapproved.", "info"); return redirect(url_for("my_requests"))
...
def cancel(req_id):
    ...
    flash("Cancelled.", "secondary"); return redirect(url_for("my_requests"))
...
def export_requests_xlsx():
    ...
    for r in rows:
        ...
        ws.write_datetime(rix, 12, r.created_at, dt_fmt) if r.created_at else ws.write(rix, 12, "", cell_fmt)
        ws.write_datetime(rix, 13, r.decided_at, dt_fmt) if r.decided_at else ws.write(rix, 13, "", cell_fmt)
        rix += 1
    ...
    wb.close(); buf.seek(0)
    return send_file(
        buf, as_attachment=True, download_name="leave_requests.xlsx",
